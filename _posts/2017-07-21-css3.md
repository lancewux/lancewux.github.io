...
- 标志@charset 定义被样式表使用的字符集
- 标志@import 告诉CSS引擎包含外部的CSS样式表
- 标志@namespace 告诉CSS引擎所有的内容都必须考虑使用XML命名空间前缀
- 嵌套at-rules
 + 标志@media 带条件的规则组，符合媒体查询则使用该规则组
 + 标志@supports 带条件的规则组，符合支持查询则使用该规则组
 + 标志@document 带条件的规则组，符合url查询则使用该规则组
 + 标志@page 描述打印时应用的样式
 + 标志@font-face 描述外部字体
 + 标志@keyframes 描述动画步骤
 + 标志@viewport 
 + 标志@counter-style
 + 标志@font-feature-values
